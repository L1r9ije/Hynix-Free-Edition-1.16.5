...
import su.hynix.events.EventCooldown;
import su.hynix.events.EventCooldownTracker;
...
public class CooldownTracker {
    ...
    public float getCooldown(Item itemIn, float partialTicks) {
        ...
        EventCooldownTracker event = new EventCooldownTracker(itemIn, 0);
    ...
    public void setCooldown(Item itemIn, int ticksIn) {
        EventCooldown event = new EventCooldown(itemIn, ticksIn);
        EventManager.call(event);
    ...
    public float getCustomCooldown(Item itemIn, float partialTicks) {
        CooldownTracker.Cooldown cooldown = this.cooldowns.get(itemIn);

        if (cooldown != null) {
            int remainingTicks = cooldown.expireTicks - (this.ticks + (int) partialTicks);

            float remainingSeconds = remainingTicks / 20.0f;

            return Math.max(remainingSeconds, 0.0f);
        } else {
            return 0.0f;
        }
    }
