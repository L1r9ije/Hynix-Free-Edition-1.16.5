...
import su.hynix.events.EventCriticalHit;
import su.hynix.events.EventGetDigSpeed;
...
public abstract class PlayerEntity extends LivingEntity {
    ...
    public float getDigSpeed(BlockState state) {
        ...
        EventGetDigSpeed event = new EventGetDigSpeed(state, f);
    ...
    public float calculateItemDigSpeed(ItemStack stack, BlockState blockState) {
        ...
        if (this.isPotionActive(Effects.MINING_FATIGUE)) {
            speed *= (switch (this.getActivePotionEffect(Effects.MINING_FATIGUE).getAmplifier()) {
                case 0 -> 0.3f;
                case 1 -> 0.09f;
                case 2 -> 0.0027f;
                default -> 8.1E-4f;
            ...
            if (!targetEntity.hitByEntity(this)) {
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    ...
                    if (flag5) {
                        ...
                        if (flag2) {
                            EventCriticalHit eventCrit = new EventCriticalHit(targetEntity);
                            EventManager.call(eventCrit);
    ...
    public boolean isPushedByWater() {
        EventNoPush eventNoPush = new EventNoPush(EventNoPush.NoPushType.Water);
    ...
    public boolean canCritical() {
        return !this.isPotionActive(Effects.LEVITATION) && !this.isPotionActive(Effects.BLINDNESS) && !this.areEyesInFluid(FluidTags.WATER) && !this.areEyesInFluid(FluidTags.LAVA) && !this.abilities.isFlying && !this.isElytraFlying() && !this.isOnLadder() && !this.isPassenger() && this.fallDistance > 0.0F && !this.isOnGround();
    }

    public boolean isValidAttackCondition() {
        return !this.isPotionActive(Effects.LEVITATION) && !this.isPotionActive(Effects.BLINDNESS) && !this.areEyesInFluid(FluidTags.WATER) && !this.areEyesInFluid(FluidTags.LAVA) && !this.isInWeb && !this.abilities.isFlying && !this.isElytraFlying() && !this.isOnLadder() && !this.isPassenger();
    }
    ...
    public static enum SleepResult {
        NOT_POSSIBLE_HERE, NOT_POSSIBLE_NOW(new TranslationTextComponent("block.minecraft.bed.no_sleep")), TOO_FAR_AWAY(new TranslationTextComponent("block.minecraft.bed.too_far_away")), OBSTRUCTED(new TranslationTextComponent("block.minecraft.bed.obstructed")), OTHER_PROBLEM, NOT_SAFE(new TranslationTextComponent("block.minecraft.bed.not_safe"));
