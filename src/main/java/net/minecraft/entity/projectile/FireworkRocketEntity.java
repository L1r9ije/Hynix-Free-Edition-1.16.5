        ...
        if (this.isAttachedToEntity()) {
            ...
            if (this.boostedEntity != null) {
                if (this.boostedEntity.isElytraFlying()) {
                    ...
                    EventFireworkRocket event = new EventFireworkRocket(1.5f, 0.1f, 0.5f, vector3d);
                    EventManager.call(event);

                    this.boostedEntity.setMotion(vector3d1.add(
                            event.getVector3d().x * event.getBaseBoost() + (event.getVector3d().x * event.getBoostMultiplier() - vector3d1.x) * event.getSmoothingFactor(),
                            event.getVector3d().y * event.getBaseBoost() + (event.getVector3d().y * event.getYSpeed() - vector3d1.y) * event.getSmoothingFactor(),
                            event.getVector3d().z * event.getBaseBoost() + (event.getVector3d().z * event.getBoostMultiplier() - vector3d1.z) * event.getSmoothingFactor()
                    ));
                }
