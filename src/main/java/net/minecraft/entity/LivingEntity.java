...
public abstract class LivingEntity extends Entity {
     ...
     */
    public double backUpX, backUpY, backUpZ, serverX, serverY, serverZ, prevServerX, prevServerY, prevServerZ;
    @Nullable
        ...
        if (this.isAlive()) {
            if (this.isEntityInsideOpaqueBlock()) {
                ...
                if (this instanceof ClientPlayerEntity) {
                    EventManager.call(new EventDamage(EventDamage.DamageType.BLOCK));
                }
    ...
    protected void onDeathUpdate() {
        if (this.deathTime < 1 && mc.player == this) {
            EventManager.call(new EventDeath());
        }
    ...
    private int getArmSwingAnimationEnd() {
        int swipeSpeed = 6;
        EventSwingSpeed event = new EventSwingSpeed(swipeSpeed, this.swingingHand);
        EventManager.call(event);
        if (event.isCancelled() && this instanceof ClientPlayerEntity) {
            return event.getSwipeSpeed();
        }
    ...
    public void travel(Vector3d travelVector) {
        EventTravel eventTravel = new EventTravel(this);
        ...
        if (this.isServerWorld() || this.canPassengerSteer()) {
            ...
            if (this.isInWater() && this.func_241208_cS_() && !this.func_230285_a_(fluidstate.getFluid())) {
                ...
                EventElytraFlying eventElytraFlying = new EventElytraFlying(this.getLookVec(), this.rotationPitch);
                EventManager.call(eventElytraFlying);

                Vector3d vector3d1 = eventElytraFlying.getVector3d();
                float f = eventElytraFlying.getF() * ((float) Math.PI / 180F);
                ...
                double dy2 = (this.getMotion().y - d0) * 0.98D;
                Vector3d attempt = new Vector3d(0.0D, dy2, 0.0D);
                Vector3d allowed = this.getAllowedMovement(attempt);
                boolean willLand = dy2 < 0.0D && allowed.y != dy2;
                EventManager.call(new EventWillLand(willLand));
            }
        }
    ...
    public boolean canBePushed() {
        EventNoPush eventNoPush = new EventNoPush(EventNoPush.NoPushType.Player);
    ...
    public boolean isEating() {
        return this.isHandActive() && this.activeItemStack.getItem().getUseAction(this.activeItemStack) == UseAction.EAT;
    }
}
