...
import su.hynix.events.EventNoRender;
import su.hynix.modules.impl.visuals.SeeInvisibles;
...
public abstract class Entity implements INameable, ICommandSource {
    ...
    public boolean canBeRaytracing() {
        EventEntityRayTrace event = new EventEntityRayTrace(this);
    ...
    public void move(MoverType typeIn, Vector3d pos) {
        if (this.noClip) {
            ...
            if (this.isInWeb) {
                this.isInWeb = false;
                pos = new Vector3d(pos.x * 0.25D, pos.y * 0.05000000074505806D, pos.z * 0.25D);
            ...
            if (this instanceof ClientPlayerEntity) {
                ...
                EventMoving moveKriper = new EventMoving(fromPosition, predictedPosition, pos, onGround, collidedHorizontally, collidedVertically, bb);
                EventManager.call(moveKriper);

                pos = moveKriper.motion();
                ignoreHorizontal = moveKriper.isIgnoreHorizontal();
                ignoreVertical = moveKriper.isIgnoreVertical();
            }
    ...
    private Vector3d getAllowedMovement(Vector3d vec, boolean horizontal, boolean vertical) {
        ...
        ReuseableStream<VoxelShape> reuseablestream = new ReuseableStream<>(Stream.concat(stream1, stream));
        Vector3d vector3d = vec.lengthSquared() == 0.0D ? vec : collideBoundingBoxHeuristically(this, vec, axisalignedbb, this.world, iselectioncontext, reuseablestream, horizontal, vertical);
        boolean flag = vec.x != vector3d.x;
        boolean flag1 = vec.y != vector3d.y;
        boolean flag2 = vec.z != vector3d.z;
        boolean flag3 = this.onGround || flag1 && vec.y < 0.0D;
        ...
        if (this.stepHeight > 0.0F && flag3 && (flag || flag2)) {
            Vector3d vector3d1 = collideBoundingBoxHeuristically(this, new Vector3d(vec.x, (double) this.stepHeight, vec.z), axisalignedbb, this.world, iselectioncontext, reuseablestream, horizontal, vertical);

            Vector3d vector3d2 = collideBoundingBoxHeuristically(this, new Vector3d(0.0D, (double) this.stepHeight, 0.0D), axisalignedbb.expand(vec.x, 0.0D, vec.z), this.world, iselectioncontext, reuseablestream, horizontal, vertical);

            if (vector3d2.y < (double) this.stepHeight) {
                Vector3d vector3d3 = collideBoundingBoxHeuristically(this, new Vector3d(vec.x, 0.0D, vec.z), axisalignedbb.offset(vector3d2), this.world, iselectioncontext, reuseablestream, horizontal, vertical).add(vector3d2);
            ...
            if (horizontalMag(vector3d1) > horizontalMag(vector3d)) {
                return vector3d1.add(collideBoundingBoxHeuristically(this, new Vector3d(0.0D, -vector3d1.y + vec.y, 0.0D), axisalignedbb.offset(vector3d1), this.world, iselectioncontext, reuseablestream, horizontal, vertical));
            }
        }
    ...
    public double getDistanceEyePos(LivingEntity target) {
        ...
        double eyeX = this.getPosX();
        double eyeY = this.getPosY() + this.getEyeHeight();
        double eyeZ = this.getPosZ();

        double closestX = eyeX < box.minX ? box.minX : eyeX > box.maxX ? box.maxX : eyeX;
        double closestY = eyeY < box.minY ? box.minY : eyeY > box.maxY ? box.maxY : eyeY;
    ...
    public double getDistanceEye(LivingEntity target) {
        double wHalf = target.getWidth() / 2.0F;
        double x = MathHelper.clamp(this.getPosX() - target.getPosX(), -wHalf, wHalf);
        double y = MathHelper.clamp(this.getPosY() + (double) this.getEyeHeight() - target.getPosY(), 0.0, target.getHeight());
    ...
    public float getCollisionBorderSize() {
        EventEntityHitBox event = new EventEntityHitBox(this, 0.0F);
        EventManager.call(event);
    ...
    public boolean isGlowing() {
        EventNoRender event = new EventNoRender(EventNoRender.NorenderType.glowing);
        EventManager.call(event);
        return !event.isCancelled() ? this.glowing || this.world.isRemote && this.getFlag(6) : false;
    }
    ...
    public boolean isInvisibleToPlayer(PlayerEntity player) {
        SeeInvisibles module = (SeeInvisibles) hynix.getInstance().getModuleManager().getModule(SeeInvisibles.class);
        if (module.isEnabled()) {
            if (this instanceof ArmorStandEntity) {
                if (module.getArmorStands().get()) {
                    return false;
                }
            } else {
                return false;
