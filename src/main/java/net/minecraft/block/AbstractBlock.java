...
import su.hynix.events.EventBlockCollide;
...
public abstract class AbstractBlock {
    ...
    public abstract static class AbstractBlockState extends StateHolder<Block, BlockState> {
        ...
        public VoxelShape getRayShape(IBlockReader worldIn, BlockPos pos, ISelectionContext context) {
            return hynix.getInstance().getModuleManager().getOpenWalls().isEnabled() && hynix.getInstance().getModuleManager().getOpenWalls().getBlockClasses().stream().noneMatch((clazz) -> clazz.isInstance(this.getBlock())) ? VoxelShapes.empty() : this.getBlock().getShape(this.getSelf(), worldIn, pos, context);
        }
        ...
        public VoxelShape getCollisionShape(IBlockReader worldIn, BlockPos pos, ISelectionContext context) {
            ...
            EventManager.call(eventBlockCollide);
            if (eventBlockCollide.isCancelled()) {
                return VoxelShapes.empty();
            }
        ...
        public boolean causesSuffocation(IBlockReader worldIn, BlockPos pos) {
            EventNoRender event = new EventNoRender(EventNoRender.NorenderType.cameraclip);
...
/*
  Изменено по запросу: "мне не нравится тут эвент потому что пока я не двинусь он не отменяет коллизию"
*/
