...
import su.hynix.modules.impl.visuals.Ambience;
...
public class FogRenderer {
    ...
    public static void updateFogColor(ActiveRenderInfo activeRenderInfoIn, float partialTicks, ClientWorld worldIn, int renderDistanceChunks, float bossColorModifier) {
        Ambience ambience = hynix.getInstance().getModuleManager().ambience;
        FluidState fluidstate = activeRenderInfoIn.getFluidState();

        if (ambience != null && ambience.isEnabled() && Ambience.fogMode.is("Переопределить") && !fluidstate.isTagged(FluidTags.WATER) && !fluidstate.isTagged(FluidTags.LAVA) && !(activeRenderInfoIn.getRenderViewEntity() instanceof LivingEntity && ((LivingEntity) activeRenderInfoIn.getRenderViewEntity()).isPotionActive(Effects.BLINDNESS))) {
            Color fogColor = new Color(Ambience.fogColor.get());
            red = fogColor.getRed() / 255.0F;
            green = fogColor.getGreen() / 255.0F;
            blue = fogColor.getBlue() / 255.0F;
        ...
        if (f >= 0.0F) {
            ...
            if (entity instanceof LivingEntity && ((LivingEntity) entity).isPotionActive(Effects.BLINDNESS)) {
                ...
                if (ambience != null && ambience.isEnabled()) {
                    if (Ambience.fogMode.is("Очистить")) {
                        RenderSystem.fogDensity(0.0F);
                        RenderSystem.fogMode(GlStateManager.FogMode.EXP2);
                        return;
                    } else if (Ambience.fogMode.is("Переопределить")) {
                        f3 = Ambience.fogStart.get() * 42.67F;
                        f4 = Ambience.fogEnd.get() * 42.67F;
