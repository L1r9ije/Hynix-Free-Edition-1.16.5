...
public class BlockRendererDispatcher implements IResourceManagerReloadListener {
    ...
    public boolean renderModel(BlockState p_renderModel_1_, BlockPos p_renderModel_2_, IBlockDisplayReader p_renderModel_3_, MatrixStack p_renderModel_4_, IVertexBuilder p_renderModel_5_, boolean p_renderModel_6_, Random p_renderModel_7_, IModelData p_renderModel_8_) {
        Block blockType = p_renderModel_1_.getBlock();
        if (blockType instanceof TallGrassBlock || blockType == Blocks.TALL_GRASS || blockType == Blocks.LARGE_FERN) {
            EventNoRender grassEvent = new EventNoRender(EventNoRender.NorenderType.grass);
            EventManager.call(grassEvent);
            if (grassEvent.isCancelled()) {
                return false;
            }
        }
        try {
