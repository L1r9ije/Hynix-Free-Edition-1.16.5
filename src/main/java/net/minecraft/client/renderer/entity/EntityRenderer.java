...
import su.hynix.modules.impl.visuals.Tags;
...
public abstract class EntityRenderer<T extends Entity> implements net.optifine.entity.model.IEntityRenderer {
    ...
    public void render(T entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        ...
        EventManager.call(renderNameplateEvent);
        if (this.canRenderName(entityIn) && renderName) {
            if (entityIn instanceof ArmorStandEntity) {
                EventNoRender hologramEvent = new EventNoRender(EventNoRender.NorenderType.holograms);
                EventManager.call(hologramEvent);
                if (!hologramEvent.isCancelled()) {
    ...
    protected void renderName(T entityIn, ITextComponent displayNameIn, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        Tags tagsModule = (Tags) hynix.getInstance().getModuleManager().getModule(Tags.class);
        if (tagsModule.isEnabled() && TargetUtil.isEntityTarget(entityIn, tagsModule.rendering))
            return;
