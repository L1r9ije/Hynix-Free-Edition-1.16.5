...
import su.hynix.modules.impl.combat.HitBox;
...
public class EntityRendererManager implements IEntityRenderManager {
    ...
    public <E extends Entity> void renderEntityStaticNoShadow(E entity, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        ...
        double d0 = MathUtil.interpolate(entity.lastTickPosX, entity.getPosX(), (double) partialTicks);
        double d1 = MathUtil.interpolate(entity.lastTickPosY, entity.getPosY(), (double) partialTicks);
        double d2 = MathUtil.interpolate(entity.lastTickPosZ, entity.getPosZ(), (double) partialTicks);
        float f = MathUtil.interpolate(entity.prevRotationYaw, entity.rotationYaw, partialTicks);
        int light = this.getPackedLight(entity, partialTicks);
        this.renderEntityStatic(entity, d0 - this.renderPosX(), d1 - this.renderPosY(), d2 - this.renderPosZ(), f, partialTicks, matrixStackIn, bufferIn, light);
    }

    public <E extends Entity> void renderEntityStaticNoShadow(E entity, float partialTicks, boolean p_147936_3_) {
        MatrixStack matrixStack = new MatrixStack();
        IRenderTypeBuffer.Impl bufferSource = Minecraft.getInstance().getRenderTypeBuffers().getBufferSource();
        int light = this.getPackedLight(entity, partialTicks);
        this.renderEntityStaticNoShadow(entity, partialTicks, matrixStack, bufferSource, light);
        bufferSource.finish();
    ...
    public <E extends Entity> void renderEntityStatic(E entityIn, double xIn, double yIn, double zIn, float rotationYawIn, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        if (this.info != null) {
            ...
            if (entityIn instanceof PlayerEntity && entityIn != Minecraft.getInstance().player) {
                EventNoRender event = new EventNoRender(EventNoRender.NorenderType.players);
            ...
            try {
                ...
                if (this.options.entityShadows && this.renderShadow && entityrenderer.shadowSize > 0.0F && !entityIn.isInvisible()) {
                    // NoRender event for shadows
                    EventNoRender event = new EventNoRender(EventNoRender.NorenderType.shadows);
                    EventManager.call(event);
                    if (!event.isCancelled()) {
    ...
    private void renderDebugBoundingBox(MatrixStack matrixStackIn, IVertexBuilder bufferIn, Entity entityIn, float partialTicks) {
        if (!Shaders.isShadowPass) {
            ...
            HitBox hitBoxModule = (HitBox) hynix.getInstance().getModuleManager().getModule(HitBox.class);
            boolean renderCustomHitBox = entityIn instanceof LivingEntity && hitBoxModule != null && hitBoxModule.shouldShowHitBox();

            if (renderCustomHitBox) {
                float customSize = HitBox.size.get();
                float modifiedWidth = (entityIn.getWidth() / 2.0F) + customSize;
                WorldRenderer.drawBoundingBox(matrixStackIn, bufferIn, -modifiedWidth, 0.0D, -modifiedWidth, modifiedWidth, entityIn.getHeight(), modifiedWidth, 1.0F, 1.0F, 1.0F, 1.0F);
            } else {
            ...
            if (entityIn instanceof LivingEntity) {
                float f1 = 0.01F;
                WorldRenderer.drawBoundingBox(matrixStackIn, bufferIn, (double) (-f), (double) (entityIn.getEyeHeight() - 0.01F), (double) (-f), (double) f, (double) (entityIn.getEyeHeight() + 0.01F), (double) f, 1.0F, 0.0F, 0.0F, 1.0F); // Eye height box
            }
