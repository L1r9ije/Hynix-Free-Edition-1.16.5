...
public class ActiveRenderInfo {
    ...
    public void update(IBlockReader worldIn, Entity renderViewEntity, boolean thirdPersonIn, boolean thirdPersonReverseIn, float partialTicks) {
        ...
        EventRotation event = new EventRotation(new Vector3d(MathHelper.lerp((double) partialTicks, renderViewEntity.prevPosX, renderViewEntity.getPosX()), MathHelper.lerp((double) partialTicks, renderViewEntity.prevPosY, renderViewEntity.getPosY()) + (double) MathHelper.lerp(partialTicks, this.previousHeight, this.height), MathHelper.lerp((double) partialTicks, renderViewEntity.prevPosZ, renderViewEntity.getPosZ())), new Vector2f(renderViewEntity.getYaw(partialTicks), renderViewEntity.getPitch(partialTicks)));
        EventManager.call(event);
        this.setDirection(event.getRotation().x, event.getRotation().y);
        this.setPosition(event.getPosition().x, event.getPosition().y, event.getPosition().z);
    ...
    private double calcCameraDistance(double startingDistance) {
        EventNoRender noRenderCameraClip = new EventNoRender(EventNoRender.NorenderType.cameraclip);
        EventManager.call(noRenderCameraClip);

        if (!noRenderCameraClip.isCancelled()) {
            for (int i = 0; i < 8; ++i) {
        ...
        EventThirdPersonDistance event = new EventThirdPersonDistance(startingDistance);
        EventManager.call(event);
