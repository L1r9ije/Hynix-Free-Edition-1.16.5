...
import su.hynix.modules.impl.visuals.Animation;
...
public class GameRenderer implements IResourceManagerReloadListener, AutoCloseable {
    ...
    private final AnimationUtil zoomAnimation = new AnimationUtil(1.0f, 12.0f, Easings.CUBIC_OUT);
    private final AnimationUtil perspectiveAnimation = new AnimationUtil(1.0f, 12.0f, Easings.CUBIC_OUT);
    ...
    public void triggerPerspectiveChangeAnimation() {
        Animation animationModule = (Animation) hynix.getInstance().getModuleManager().getModule(Animation.class);
        if (animationModule == null || !animationModule.isEnabled()) return;
        Boolean enabled = animationModule.mode.is("Изменение перспективы");
        if (!Boolean.TRUE.equals(enabled)) return;
        this.perspectiveAnimation.setValue(0.7f);
        this.perspectiveAnimation.update(1.0f);
    }
    ...
    public void setupOverlayRendering(float scale) {
        ...
        double scaledWidth = mw.getWidth() / mw.getGuiScaleFactor();
        double scaledHeight = mw.getHeight() / mw.getGuiScaleFactor();
        if (scaledWidth <= 0 || scaledHeight <= 0) {
            scaledWidth = Math.max(1.0, mw.getWidth());
            scaledHeight = Math.max(1.0, mw.getHeight());
        }
    ...
    public void getMouseOver(float partialTicks) {
        ...
        if (entity != null && this.mc.world != null) {
            ...
            EntityRayTraceResult entityraytraceresult = ProjectileHelper.rayTraceEntities(entity, vector3d, vector3d2, axisalignedbb, (p_lambda$getMouseOver$0_0_) -> {
                return p_lambda$getMouseOver$0_0_.canBeRaytracing() && !p_lambda$getMouseOver$0_0_.isSpectator() && p_lambda$getMouseOver$0_0_.canBeCollidedWith();
            }, d1);
    ...
    public double getFOVModifier(ActiveRenderInfo activeRenderInfoIn, float partialTicks, boolean useFOVSetting) {
        if (this.debugView) {
            ...
            boolean zoomAnimEnabled = false;
            Animation animationModule = (Animation) hynix.getInstance().getModuleManager().getModule(Animation.class);
            if (animationModule != null && animationModule.isEnabled()) {
                Boolean enabled = animationModule.mode.is("Приближение камеры");
                zoomAnimEnabled = Boolean.TRUE.equals(enabled);
            }

            if (zoomAnimEnabled) {
                float target = flag ? 0.25f : 1.0f;
                this.zoomAnimation.update(target);
                d0 *= this.zoomAnimation.getValue();
            } else if (flag && Config.zoomMode) {
                d0 /= 4.0D;
            }

            // Apply perspective switch animation multiplier
            boolean perspectiveAnimEnabled = false;
            Animation animationModule2 = (Animation) hynix.getInstance().getModuleManager().getModule(Animation.class);
            if (animationModule2 != null && animationModule2.isEnabled()) {
                Boolean enabled2 = animationModule2.mode.is("Изменение перспективы");
                perspectiveAnimEnabled = Boolean.TRUE.equals(enabled2);
            }

            if (perspectiveAnimEnabled && this.perspectiveAnimation.isAlive()) {
                this.perspectiveAnimation.update(1.0f);
                d0 *= MathHelper.clamp(this.perspectiveAnimation.getValue(), 0.5f, 1.0f);
            }
    ...
    private void hurtCameraEffect(MatrixStack matrixStackIn, float partialTicks) {
        EventNoRender event = new EventNoRender(EventNoRender.NorenderType.hurttime);
    ...
    public void renderHand(MatrixStack p_renderHand_1_, ActiveRenderInfo p_renderHand_2_, float p_renderHand_3_, boolean p_renderHand_4_, boolean p_renderHand_5_, boolean p_renderHand_6_) {
        if (!this.debugView) {
            ...
            if (p_renderHand_4_) {
                ...
                EventThirdPersonRender eventThirdPersonRender = new EventThirdPersonRender(this.mc.gameSettings.getPointOfView().firstPerson());
                EventManager.call(eventThirdPersonRender);

                if (eventThirdPersonRender.isThirdperson() && !flag && !this.mc.gameSettings.hideGUI && this.mc.playerController.getCurrentGameType() != GameType.SPECTATOR) {
                    this.lightmapTexture.enableLightmap();


                    EventManager.call(new EventHandsRender.Pre(activeRender, p_renderHand_1_, p_renderHand_3_));
                    if (Config.isShaders()) {
                    ...
                    EventManager.call(new EventHandsRender.Post(activeRender, p_renderHand_1_, p_renderHand_3_));
    ...
    public Matrix4f getProjectionMatrix(ActiveRenderInfo activeRenderInfoIn, float partialTicks, boolean useFovSetting) {
        ...
        float aspectRatio = (float) this.mc.getMainWindow().getFramebufferWidth() / (float) this.mc.getMainWindow().getFramebufferHeight();
        EventAspectRatio event = new EventAspectRatio(aspectRatio);
        EventManager.call(event);

        if (!event.isCancelled()) {
            aspectRatio = event.getAspectRatio();
        }
    ...
    private void frameInit() {
        ...
        if (this.updatedWorld != world) {
            ...
            EventManager.call(new EventSwapWorld(EventSwapWorld.State.CHANGED));
        }
