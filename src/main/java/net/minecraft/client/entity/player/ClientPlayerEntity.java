...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
    ...
    public void tick() {
        if (this.world.isBlockLoaded(new BlockPos(this.getPosX(), 0.0D, this.getPosZ()))) {
            ...
            if (this.openContainer != null) {
                ...
                for (Slot s : this.openContainer.inventorySlots) {
                    list.add(s.getStack().copy());
                }
                EventManager.call(new EventContainerTick(this.openContainer.windowId, list));
            }
    ...
    private void onUpdateWalkingPlayer() {
        ...
        EventMotion event = new EventMotion(posX, posY, posZ, yaw, pitch, onGround, isSneaking, isSprinting);
        EventManager.call(event);

        if (event.isSprinting() != this.serverSprintState) {
            CEntityActionPacket.Action centityactionpacket$action = event.isSprinting() ? CEntityActionPacket.Action.START_SPRINTING : CEntityActionPacket.Action.STOP_SPRINTING;
            this.connection.sendPacket(new CEntityActionPacket(this, centityactionpacket$action));
            this.serverSprintState = event.isSprinting();
        }

        boolean flag3 = event.isSneaking();
        ...
        if (this.isCurrentViewEntity()) {
            ...
            if (this.isPassenger()) {
                ...
                this.connection.sendPacket(new CPlayerPacket.PositionRotationPacket(event.getX(), event.getY(), event.getZ(), event.getYaw(), event.getPitch(), event.isOnGround()));
            } else if (flag1) {
    ...
    public void sendChatMessage(String message) {
        if (message.startsWith(hynix.getInstance().getCommandsManager().getPrefix())) {
            try {
                hynix.getInstance().getCommandsManager().getDispatcher().execute(
                        message.substring(hynix.getInstance().getCommandsManager().getPrefix().length()),
                        hynix.getInstance().getCommandsManager().getSource()
                );
    ...
    public void closeScreenAndDropStack() {
        ...
        super.closeScreen();
        if (!(this.mc.currentScreen instanceof su.hynix.ui.gui.DropDown)) {
    ...
    public void updateEntityActionState() {
        ...
        if (this.isCurrentViewEntity()) {
            ...
            this.renderArmPitch = (float) ((double) this.renderArmPitch + (double) (LookHandler.getFreePitch() - this.renderArmPitch) * 0.5D);
            this.renderArmYaw = (float) ((double) this.renderArmYaw + (double) (LookHandler.getFreeYaw() - this.renderArmYaw) * 0.5D);
        }
    }
    ...
    public void livingTick() {
        ...
        boolean allowFlying = this.abilities.allowFlying;
        IBaritone baritone = BaritoneAPI.getProvider().getBaritoneForPlayer(this);
        if (baritone != null) {
            allowFlying = !baritone.getPathingBehavior().isPathing() && this.abilities.allowFlying;
        }

        boolean flag4 = (float) this.getFoodStats().getFoodLevel() > 6.0F || allowFlying;
        ...
        if (!this.isSprinting() && (!this.isInWater() || this.canSwim()) && this.isUsingSwimmingAnimation() && flag4 && !this.isHandActive() && !this.isPotionActive(Effects.BLINDNESS)) {
            boolean isSprintKeyDown = this.mc.gameSettings.keyBindSprint.isKeyDown();
            if (baritone != null) {
    ...
    private void handlePortalTeleportation() {
        ...
        if (this.inPortal) {
            ...
            EventNoRender event = new EventNoRender(EventNoRender.NorenderType.bad_effects);
            EventManager.call(event);
            if (!event.isCancelled()) {
