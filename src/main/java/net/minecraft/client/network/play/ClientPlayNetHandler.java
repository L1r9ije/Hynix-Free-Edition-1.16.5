...
import su.hynix.events.EventItemPickup;
import su.hynix.events.EventNoRender;
import su.hynix.events.EventNoRotate;
import su.hynix.events.EventPopTotem;
...
public class ClientPlayNetHandler implements IClientPlayNetHandler {
    ...
    public void handleJoinGame(SJoinGamePacket packetIn) {
        ...
        EventManager.call(new ClientPlayerNetworkEvent.LoggedInEvent(mc.playerController, mc.player, mc.networkManager));
    ...
    public void handlePlayerPosLook(SPlayerPositionLookPacket packetIn) {
        ...
        EventManager.call(eventNoRotate);
        if (eventNoRotate.isCancelled()) {
            playerentity.setPosition(d1, d3, d5);
        } else {
            playerentity.setPositionAndRotation(d1, d3, d5, eventNoRotate.getYaw(), eventNoRotate.getPitch());
        }
    ...
    public void handleCollectItem(SCollectItemPacket packetIn) {
        ...
        if (entity != null) {
            ...
            if (entity instanceof ItemEntity) {
                ...
                EventItemPickup event = new EventItemPickup(itementity, livingentity, itemstack.copy(), packetIn.getAmount());
    ...
    public void handleChat(SChatPacket packetIn) {
        ...
        String msg = packetIn.getChatComponent().getString();
        if (msg.contains("Эй! Извините, но вы не можете сломать блок здесь.")) {
            for (IBaritone ibar : BaritoneAPI.getProvider().getAllBaritones()) {
                MineProcess mineProc = (MineProcess) ibar.getMineProcess();
                if (mineProc.isActive()) {
                    mineProc.onProtectedBlockMessage();
                }
            }
        }
    }
    ...
    public void handleEntityStatus(SEntityStatusPacket packetIn) {
        ...
        if (entity != null) {
            if (packetIn.getOpCode() == 21) {
                ...
                EventPopTotem pop = new EventPopTotem(entity);
                EventManager.call(pop);
                ...
                EventNoRender event = new EventNoRender(EventNoRender.NorenderType.totem);
                EventManager.call(event);
                if (entity == this.client.player && !event.isCancelled()) {
    ...
    public void handleSetSlot(SSetSlotPacket packetIn) {
        ...
        if (packetIn.getWindowId() == -1) {
            ...
            if (packetIn.getWindowId() == 0 && i >= 0 && i < 45) {
                if (!itemstack.isEmpty()) {
