...
import su.hynix.events.EventClickBlockRight;
import su.hynix.events.EventWindowClick;
...
public class PlayerController implements IPlayerControllerMP {
    ...
    public ActionResultType func_217292_a(ClientPlayerEntity p_217292_1_, ClientWorld p_217292_2_, Hand p_217292_3_, BlockRayTraceResult p_217292_4_) {
        EventClickBlockRight event = new EventClickBlockRight(p_217292_1_, p_217292_2_, p_217292_3_, p_217292_4_);
    ...
    public ItemStack windowClick(int windowId, int slotId, int mouseButton, ClickType type, PlayerEntity player) {
        ...
        EventWindowClick event = new EventWindowClick(windowId, slotId, mouseButton, type, itemstack, short1);
        EventManager.call(event);
        if (!event.isCancelled()) {
            this.connection.sendPacket(new CClickWindowPacket(event.getWindowId(), event.getSlotId(), event.getMouseButton(), event.getType(), event.getClickedItemIn(), event.getActionNumberIn()));
        }
        return itemstack;
    }
