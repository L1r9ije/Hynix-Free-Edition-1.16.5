...
public abstract class ContainerScreen<T extends Container> extends Screen implements IHasContainer<T> {
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        EventManager.call(new EventContainerRender.Pre(matrixStack, this.guiLeft, this.guiTop, this.container));
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (super.mouseClicked(mouseX, mouseY, button)) {
            ...
            //чек нбт
            if (button == 2 && slot != null && slot.getHasStack()) {
                System.out.println(slot.getStack().getTag() != null ? slot.getStack().getTag().toString() : "Нет NBT");
                return true;
            }
