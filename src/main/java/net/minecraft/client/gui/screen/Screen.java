...
import su.hynix.events.EventRenderTooltip;
...
public abstract class Screen extends FocusableGui implements IScreen, IRenderable, IGuiScreen {
    ...
    protected void renderTooltip(MatrixStack matrixStack, ItemStack itemStack, int mouseX, int mouseY) {
        EventRenderTooltip event = new EventRenderTooltip(matrixStack, itemStack, mouseX, mouseY);
    ...
    public boolean handleComponentClicked(@Nullable Style style) {
        if (style.getClickEvent() instanceof EventClientClick clientClickEvent) {
            String value = clientClickEvent.getValue();
            String prefix = hynix.getInstance().getCommandsManager().getPrefix();
            CommandManager manager = hynix.getInstance().getCommandsManager();
            try {
                if (value.startsWith(prefix)) {
                    manager.getDispatcher().execute(value.substring(prefix.length()), manager.getSource());
                } else {
                    manager.getDispatcher().execute(value, manager.getSource());
                }
                return true;
    ...
    public void renderBackground(MatrixStack matrixStack, int vOffset) {
        EventNoRender event = new EventNoRender(EventNoRender.NorenderType.container_background);
        EventManager.call(event);
