...
public class NewChatGui extends AbstractGui {
    ...
    private String nlf_lastMsg = "";
    private int nlf_msgCount = 0, nlf_msgId = 0;
    ...
    public void clearChatMessages(boolean clearSentMsgHistory) {
        ...
        nlf_lastMsg = "";
        nlf_msgCount = nlf_msgId = 0;
        nextMsgId = 0;
    }

    public void printChatMessage(ITextComponent chatComponent) {
        ChatHelper chat = hynix.getInstance().getModuleManager().chatHelper;
        String plain = chatComponent.getString();

        if (!chat.isEnabled() || !chat.antiSpam.get()) {
            nlf_lastMsg = "";
            nlf_msgCount = nlf_msgId = 0;
        ...
        if (plain.equals(nlf_lastMsg)) {
            if (nlf_msgId == 0) nlf_msgId = ++nextMsgId;
            chatComponent = chatComponent.deepCopy().append(new StringTextComponent(" (" + (++nlf_msgCount) + ")").mergeStyle(TextFormatting.GRAY));
        } else {
            nlf_lastMsg = plain;
            nlf_msgCount = 1;
            nlf_msgId = ++nextMsgId;
        }
        printChatMessageWithOptionalDeletion(chatComponent, nlf_msgId);
    }
