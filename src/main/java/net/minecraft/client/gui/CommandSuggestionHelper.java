...
public class CommandSuggestionHelper {
    ...
    public void init() {
        ...
        if (reader.canRead(hynix.getInstance().getCommandsManager().getPrefix().length()) && reader.getString().startsWith(hynix.getInstance().getCommandsManager().getPrefix(), reader.getCursor())) {
            reader.setCursor(reader.getCursor() + 1);

            if (parseResults == null) {
                parseResults = hynix.getInstance().getCommandsManager().getDispatcher().parse(reader, hynix.getInstance().getCommandsManager().getSource());
            }

            final int cursor = inputField.getCursorPosition();

            if (cursor >= 1 && (suggestions == null || !isApplyingSuggestion)) {
                suggestionsFuture = hynix.getInstance().getCommandsManager().getDispatcher().getCompletionSuggestions(parseResults, cursor);
        ...
        boolean wantCommands = this.commandsOnly || hasSlash;
        int cursor = this.inputField.getCursorPosition();

        if (wantCommands) {
            CommandDispatcher<ISuggestionProvider> dispatcher = this.mc.player.connection.getCommandDispatcher();

            if (this.parseResults == null) {
                this.parseResults = dispatcher.parse(reader, this.mc.player.connection.getSuggestionProvider());
            }

            int parseCursor = this.hasCursor ? reader.getCursor() : 1;
            if (cursor >= parseCursor && (this.suggestions == null || !this.isApplyingSuggestion)) {
                this.suggestionsFuture = dispatcher.getCompletionSuggestions(this.parseResults, cursor);
            ...
            int ws = getLastWhitespace(before);
            Collection<String> names = this.mc.player.connection.getSuggestionProvider().getPlayerNames();
            ...
            if (evt.completions != null) {
                List<Suggestion> list = Stream.of(evt.completions).map(c -> new Suggestion(StringRange.between(ws, before.length()), c)).collect(Collectors.toList());
                this.suggestionsFuture = CompletableFuture.completedFuture(new com.mojang.brigadier.suggestion.Suggestions(StringRange.between(ws, before.length()), list));
            } else {
                this.suggestionsFuture = ISuggestionProvider.suggest(names, new SuggestionsBuilder(before, ws));
            }
        }
    }
        ...
        if (this.inputField.getCursorPosition() == this.inputField.getText().length()) {
            if (this.suggestionsFuture.join().isEmpty() && !this.parseResults.getExceptions().isEmpty()) {
                ...
                for (Entry<CommandNode<ISuggestionProvider>, CommandSyntaxException> entry : this.parseResults.getExceptions().entrySet()) {
                    CommandSyntaxException ex = entry.getValue();
                    if (ex.getType() == CommandSyntaxException.BUILT_IN_EXCEPTIONS.literalIncorrect()) {
                        ++i;
                    } else {
                        this.exceptionList.add(func_243255_a(ex));
                    }
                }
                if (i > 0) {
