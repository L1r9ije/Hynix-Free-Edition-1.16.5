...
import su.hynix.events.EventHotbarRender;
...
public class IngameGui extends AbstractGui {
    ...
    /** The current title displayed */ private ITextComponent displayedTitle;
    @Nullable

    /** The current sub-title displayed */ private ITextComponent displayedSubTitle;
    /**
    ...
    public void renderIngameGui(MatrixStack matrixStack, float partialTicks) {
        ...
        EventManager.call(new EventRender2D(matrixStack, partialTicks));
        EventManager.call(new EventRender2D.Pre(matrixStack, partialTicks));
        EventManager.call(new EventRender2D.Post(matrixStack, partialTicks));
        EventManager.call(new EventRender2D.Send(matrixStack, partialTicks));
        ...
        if (!this.mc.gameSettings.hideGUI) {
            ...
            EventCrosshair eventCrosshair = new EventCrosshair(matrixStack, partialTicks);
            EventManager.call(eventCrosshair);
            if (!eventCrosshair.isCancelled()) this.func_238456_d_(matrixStack);
        ...
        if (!this.mc.gameSettings.hideGUI) {
            ...
            EventNoRender event = new EventNoRender(EventNoRender.NorenderType.title);
            ...
            boolean wantsVisible = this.mc.gameSettings.keyBindPlayerList.isKeyDown() && (!this.mc.isIntegratedServerRunning() || this.mc.player.connection.getPlayerInfoMap().size() > 1 || scoreobjective1 != null);
            this.overlayPlayerList.setVisible(wantsVisible);
            if (this.overlayPlayerList.shouldRender()) {
    ...
    private void func_238456_d_(MatrixStack p_238456_1_) {
        ...
        if (gamesettings.getPointOfView().firstPerson() && (this.mc.playerController.getCurrentGameType() != GameType.SPECTATOR || this.isTargetNamedMenuProvider(this.mc.objectMouseOver)) || hynix.getInstance().getModuleManager().getModule(FreeCamera.class).isEnabled()) {
    ...
    private void func_238447_a_(MatrixStack p_238447_1_, ScoreObjective p_238447_2_) {
        EventNoRender event = new EventNoRender(EventNoRender.NorenderType.scoreboard);
        EventManager.call(event);
        if (event.isCancelled()) {
            return;
        }
    ...
    private void func_238457_e_(MatrixStack p_238457_1_) {
        ...
        if (playerentity != null) {
            ...
            EventNoRender healthEvent = new EventNoRender(EventNoRender.NorenderType.health);
            EventManager.call(healthEvent);
            int k3 = -1;
            if (!healthEvent.isCancelled() && playerentity.isPotionActive(Effects.REGENERATION)) {
            ...
            if (j6 == 0) {
                ...
                if (!playerentity.isCreative() && foodstats.getFoodLevel() >= 20 && foodstats.getSaturationLevel() >= 0.5F) {
                    this.mc.getTextureManager().bindTexture(AbstractGui.GUI_ICONS_LOCATION);
                    ...
                    for (int idx = 0; idx < totalIcons; ++idx) {
                        ...
                        } else if (idx == fullIcons && (hasHalfIcon || saturation < 1.0F)) {
            ...
            if (playerentity.areEyesInFluid(FluidTags.WATER) || j7 < l6) {
                ...
                l2 = l2 - l7 * 10;
                if (foodstats.getSaturationLevel() >= 0.5F) {
                    l2 -= 8;
                }
    ...
    private void renderPumpkinOverlay() {
        EventNoRender pumpkinEvent = new EventNoRender(EventNoRender.NorenderType.vignette);
        EventManager.call(pumpkinEvent);
        if (pumpkinEvent.isCancelled()) {
            return;
        }
        RenderSystem.disableDepthTest();
    ...
    private void renderVignette(Entity entityIn) {
        EventNoRender vignetteEvent = new EventNoRender(EventNoRender.NorenderType.vignette);
        EventManager.call(vignetteEvent);
        if (vignetteEvent.isCancelled()) {
            return;
        }
        if (!Config.isVignetteEnabled()) {
            RenderSystem.enableDepthTest();
