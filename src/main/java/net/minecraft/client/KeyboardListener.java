...
public class KeyboardListener {
    ...
    private final Set<Integer> gameplayPressedKeys = new HashSet<>();
    ...
    public void onKeyEvent(long windowPointer, int key, int scanCode, int action, int modifiers) {
        if (windowPointer == this.mc.getMainWindow().getHandle()) {
            ...
            if (!(this.mc.currentScreen instanceof ControlsScreen) || ((ControlsScreen) inestedguieventhandler).time <= Util.milliTime() - 20L) {
                ...
                if (action == 0 && mc.currentScreen == null) {
                    if (gameplayPressedKeys.remove(key)) {
                        EventManager.call(new EventKey(key, false));
                    }
                } else if (action == 0) {
                    gameplayPressedKeys.remove(key);
                }
            }
            ...
            EventManager.call(new InputEvent.KeyInputEvent(key, scanCode, action, modifiers));
