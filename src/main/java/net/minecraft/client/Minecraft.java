...
import su.hynix.modules.impl.miscellaneous.ChatHelper;
...
public class Minecraft extends RecursiveEventLoop<Runnable> implements ISnooperInfo, IWindowEventListener {
    ...
    private static final long HOOK_INTERVAL_MS = 1000000000L / 60; // 60 Fps (?13.37 ms)
    ...
    public void displayGuiScreen(@Nullable Screen guiScreenIn) {
        ...
        if (guiScreenIn instanceof MainMenuScreen || guiScreenIn instanceof MultiplayerScreen) {
            ...
            if (!(hynix.getInstance().getModuleManager().chatHelper.isEnabled() && ChatHelper.chatHistory.get())) {
    ...
    private void runGameLoop(boolean renderWorldIn) {
        ...
        long nanoTime = Util.nanoTime();
        {
            ...
            accumulatedCalls += (int) (elapsed / HOOK_INTERVAL_MS);
            lastHookTime += (accumulatedCalls * HOOK_INTERVAL_MS);
            accumulatedCalls = Math.min(accumulatedCalls, 65);
            while (accumulatedCalls > 0) {
                EventGameUpdate eventGameUpdate = new EventGameUpdate();
                EventManager.call(eventGameUpdate);
                accumulatedCalls--;
            }
        }
        ...
        if (this.leftClickCounter <= 0 && !this.player.isHandActive()) {
            if (leftClick && this.objectMouseOver != null && this.objectMouseOver.getType() == RayTraceResult.Type.BLOCK) {
                ...
                if (!this.world.getBlockState(blockpos).isAir()) {
                    ...
                    EventManager.call(new EventBlockDamage(this.world.getBlockState(blockpos), blockpos, EventBlockDamage.State.START));
                    ...
                    EventManager.call(new EventBlockDamage(this.world.getBlockState(blockpos), blockpos, EventBlockDamage.State.STOP));
                }
            } else {
        ...
        if (!this.playerController.getIsHittingBlock()) {
            ...
            if (!this.player.isRowingBoat()) {
                ...
                for (Hand hand : Hand.values()) {
                    ...
                    if (this.objectMouseOver != null) {
                        switch (this.objectMouseOver.getType()) {
                            case ENTITY:
                                ...
                                EventInteract event = new EventInteract(entity);
                            ...
                            case BLOCK:
                                RayTraceResult rayTraceResult = Rotation.rayTrace(playerController.getBlockReachDistance(), this.player.rotationYaw, this.player.rotationPitch, player, RayTraceContext.BlockMode.OPENWALLS);
                                if (rayTraceResult.getType() == RayTraceResult.Type.BLOCK && rayTraceResult instanceof BlockRayTraceResult traceResult && hynix.getInstance().getModuleManager().getOpenWalls().isEnabled() && player.getHeldItemMainhand().getItem() != Items.EXPERIENCE_BOTTLE) {
                                    ActionResultType actionresulttype1 = playerController.func_217292_a(player, world, hand, traceResult);
                    ...
                    EventRightClickItemCheck event = new EventRightClickItemCheck(itemstack, hand);
                    EventManager.call(event);
                    if (event.isCancelled()) {
                        return;
                    }
    ...
    public void runTick() {
        ...
        EventManager.call(new mods.voicechat.eventforge.TickEvent.ClientTickEvent(mods.voicechat.eventforge.TickEvent.Phase.START));
        ...
        if (this.loadingGui == null && (this.currentScreen == null || this.currentScreen.passEvents || passEvents(currentScreen))) {
        ...
        this.profiler.endSection();
        EventManager.call(new mods.voicechat.eventforge.TickEvent.ClientTickEvent(mods.voicechat.eventforge.TickEvent.Phase.END));
    }
    ...
    private void processKeyBinds() {
        for (; this.gameSettings.keyBindTogglePerspective.isPressed(); this.worldRenderer.setDisplayListEntitiesDirty()) {
            EventCancelThirdPerson eventCancelThirdPerson = new EventCancelThirdPerson();
            EventManager.call(eventCancelThirdPerson);
            if (eventCancelThirdPerson.isCancelled()) {
                continue;
            }
            ...
            PointOfView newPOV = this.gameSettings.getPointOfView();
            if (!newPOV.firstPerson() && !newPOV.thirdPersonFront()) {
                this.gameRenderer.triggerPerspectiveChangeAnimation();
            }
        }
        ...
        for (int i = 0; i < 9; ++i) {
            ...
            if (this.gameSettings.keyBindsHotbar[i].isPressed()) {
                if (this.player.isSpectator()) {
                    ...
                    EventCancelHotbar eventCancelHotbar = new EventCancelHotbar();
                    EventManager.call(eventCancelHotbar);
                    if (!eventCancelHotbar.isCancelled()) this.player.inventory.currentItem = i;
                } else {
        ...
        while (this.gameSettings.keyBindDrop.isPressed()) {
            EventDropItem eventDropItem = new EventDropItem(this.player.inventory.currentItem);
            EventManager.call(eventDropItem);
            if (!eventDropItem.isCancelled() && !this.player.isSpectator() && this.player.drop(Screen.hasControlDown())) {
                this.player.swingArm(Hand.MAIN_HAND);
            }
        }
    ...
    public void loadWorld(ClientWorld worldClientIn) {
        ...
        if (this.world != null) {
            EventManager.call(new mods.voicechat.eventforge.WorldEvent.Unload(this.world));
        }
        ...
        EventSwapWorld eventWorld = new EventSwapWorld(EventSwapWorld.State.LOAD);
        EventManager.call(eventWorld);
    }
    ...
    public void unloadWorld(Screen screenIn) {
        ...
        EventManager.call(new ClientPlayerNetworkEvent.LoggedOutEvent(playerController, player, player != null ? (player.connection != null ? player.connection.getNetworkManager() : null) : null));
