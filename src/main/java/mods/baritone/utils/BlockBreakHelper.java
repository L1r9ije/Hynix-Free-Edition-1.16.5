        ...
        if (ctx.minecraft().leftClickCounter <= 0 && ctx.player() != null && !ctx.player().isHandActive()) {
            if (isLeftClick && trace != null && trace.getType() == RayTraceResult.Type.BLOCK) {
                ...
                if (!ctx.world().getBlockState(blockPos).isAir()) {
                    ...
                    EventManager.call(new EventBlockDamage(ctx.world().getBlockState(blockPos), blockPos, EventBlockDamage.State.START));
                    ...
                    EventManager.call(new EventBlockDamage(ctx.world().getBlockState(blockPos), blockPos, EventBlockDamage.State.STOP));
                }
            } else {
                ctx.playerController().resetBlockRemoving();
            }
