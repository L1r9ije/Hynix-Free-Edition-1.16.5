...
public class ViaMCP {
    ...
    public ViaMCP() {
        ViaLoadingBase.ViaLoadingBaseBuilder.create().runDirectory(new File("ViaMCP")).nativeVersion(NATIVE_VERSION).onProtocolReload(protocolVersion -> {
            if (versionSelectScreen != null) {
                versionSelectScreen.setVersion(protocolVersion.getVersion());
            }
        }).build();
        this.versionSelectScreen = new VersionSelectScreen(Minecraft.getInstance().fontRenderer, 5, 5, 75, 20, ITextComponent.getTextComponentOrEmpty(ProtocolVersion.getProtocol(NATIVE_VERSION).getName()));
        fixTransactions();
    }
