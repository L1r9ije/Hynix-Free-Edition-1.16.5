...
import mods.voicechat.gui.widgets.*;
...
public class VoiceChatSettingsScreen extends VoiceChatScreenBase {
    ...
    protected void init() {
        ...
        keybindButton = new KeybindButton(KEY_PTT, guiLeft + 10 + 20 + 1, y + 21, xSize - 20 - 20 - 1, 20, PUSH_TO_TALK);
        ...
        addButton(new Button(guiLeft + 10, y, xSize / 2 - 15, 20, SELECT_MICROPHONE, button -> {
            minecraft.displayGuiScreen(new SelectMicrophoneScreen(this));
        }));
        addButton(new Button(guiLeft + xSize / 2 + 1, y, (xSize - 20) / 2 - 1, 20, SELECT_SPEAKER, button -> {
            minecraft.displayGuiScreen(new SelectSpeakerScreen(this));
        }));
        y += 21;
    ...
    public void renderForeground(MatrixStack poseStack, int mouseX, int mouseY, float delta) {
        ...
        font.func_238422_b_(poseStack, TITLE.func_241878_f(), (float) (guiLeft + (xSize - titleWidth) / 2), guiTop + 7, getFontColor());
