...
public class JoinGroupScreen extends ListScreenBase {
    ...
    protected void init() {
        ...
        createGroup = new Button(guiLeft + 7, guiTop + ySize - 20 - 7, xSize - 14, 20, CREATE_GROUP, button -> {
            minecraft.displayGuiScreen(new CreateGroupScreen());
        });
    ...
    public void renderForeground(MatrixStack poseStack, int mouseX, int mouseY, float delta) {
        font.func_243248_b(poseStack, JOIN_CREATE_GROUP, guiLeft + xSize / 2 - font.getStringPropertyWidth(JOIN_CREATE_GROUP) / 2, guiTop + 5, FONT_COLOR);

        if (!groupList.isEmpty()) {
            groupList.render(poseStack, mouseX, mouseY, delta);
        } else {
            drawCenteredString(poseStack, font, NO_GROUPS, width / 2, guiTop + HEADER_SIZE + (units * UNIT_SIZE) / 2 - font.FONT_HEIGHT / 2, -1);
        }
    }
