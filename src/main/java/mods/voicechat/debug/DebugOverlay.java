...
import mods.voicechat.voice.client.*;
...
public class DebugOverlay {
    ...
    private void render(MatrixStack stack, float tickDelta) {
        ...
        rightText.add(String.format("Press ALT + %s to toggle", ClientCompatibilityManager.INSTANCE.getBoundKeyOf(KEY_VOICE_CHAT).func_237520_d_().getString()));
        rightText.add(null);
    ...
    private void drawRight(MatrixStack stack, List<String> strings) {
        for (int i = 0; i < strings.size(); i++) {
            ...
            stack.translate(mc.getMainWindow().getScaledWidth() - width - LEFT_PADDING, 25F + i * (mc.fontRenderer.FONT_HEIGHT + 1F), 0F);
            AbstractGui.fill(stack, -1, -1, width, mc.fontRenderer.FONT_HEIGHT, 0x90505050);
            mc.fontRenderer.drawString(stack, text, 0, 0, 0xFFFFFF);
