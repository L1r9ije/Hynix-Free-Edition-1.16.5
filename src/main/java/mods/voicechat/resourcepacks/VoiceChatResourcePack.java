...
public class VoiceChatResourcePack extends ResourcePack {
    ...
    public ResourcePackInfo toPack() {
        try {
            PackMetadataSection packMetadataSection = getMetadata(PackMetadataSection.SERIALIZER);
            if (packMetadataSection == null) {
                return null;
            }
            return new ResourcePackInfo(path, false, () -> this, name, packMetadataSection.getDescription(), PackCompatibility.COMPATIBLE, ResourcePackInfo.Priority.TOP, false, IPackNameDecorator.BUILTIN);
        } catch (IOException e) {
            return null;
        }
    }
    ...
    @Override
    public Set<String> getResourceNamespaces(ResourcePackType packType) {
