...
public class Server extends Thread {
    ...
    private String getBindAddress() {
        ...
        if (bindAddress.trim().equals("*")) {
            ...
            if (server instanceof DedicatedServer) {
                bindAddress = ((DedicatedServer) server).getServerProperties().serverIp;
    ...
    public void onMicPacket(UUID playerUuid, MicPacket packet) {
        ...
        if (!PermissionManager.INSTANCE.SPEAK_PERMISSION.hasPermission(player)) {
            CooldownTimer.run("no-speak-" + playerUuid, 30_000L, () -> {
                player.sendStatusMessage(new TranslationTextComponent("message.voicechat.no_speak_permission"), true);
            });
            return;
        }
    ...
    private void processGroupPacket(PlayerState senderState, ServerPlayerEntity sender, MicPacket packet) {
        ...
        for (PlayerState state : playerStateManager.getStates()) {
            ...
            ServerPlayerEntity p = server.getPlayerList().getPlayerByUUID(state.getUuid());
            if (p == null) {
                continue;
            }
    ...
    private void processProximityPacket(PlayerState senderState, ServerPlayerEntity sender, MicPacket packet) {
        ...
        if (sender.isSpectator()) {
            ...
            if (Voicechat.SERVER_CONFIG.spectatorInteraction.get()) {
                soundPacket = new LocationSoundPacket(sender.getUniqueID(), sender.getUniqueID(), sender.getEyePosition(1F), packet.getData(), packet.getSequenceNumber(), distance, null);
        ...
        broadcast(ServerWorldUtils.getPlayersInRange(sender.getServerWorld(), sender.getPositionVec(), getBroadcastRange(distance), p -> !p.getUniqueID().equals(sender.getUniqueID())), soundPacket, sender, senderState, groupId, source);
    }
    ...
    public void sendSoundPacket(@Nullable ServerPlayerEntity sender, @Nullable PlayerState senderState, ServerPlayerEntity receiver, PlayerState receiverState, @Nullable ClientConnection connection, SoundPacket<?> soundPacket, String source) {
        ...
        if (!PermissionManager.INSTANCE.LISTEN_PERMISSION.hasPermission(receiver)) {
            CooldownTimer.run(String.format("no-listen-%s", receiver.getUniqueID()), 30_000L, () -> {
                receiver.sendStatusMessage(new TranslationTextComponent("message.voicechat.no_listen_permission"), true);
            });
            return;
        }
