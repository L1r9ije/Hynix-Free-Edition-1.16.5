...
public class ClientCategoryManager extends CategoryManager {
    ...
    private void registerImage(String id, NativeImage image) {
        ResourceLocation resourceLocation = Minecraft.getInstance().getRenderManager().textureManager.getDynamicTextureLocation(id, new DynamicTexture(image));
    ...
    private void unRegisterImage(String id) {
        ...
        if (resourceLocation != null) {
            Minecraft.getInstance().getRenderManager().textureManager.deleteTexture(resourceLocation);
            images.remove(id);
        }
    }
