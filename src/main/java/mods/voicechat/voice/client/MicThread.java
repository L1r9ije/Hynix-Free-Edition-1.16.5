...
public class MicThread extends Thread {
    ...
    private void flush() {
        ...
        recorder.flushChunkThreaded(Minecraft.getInstance().getSession().getProfile().getId());
    }
    ...
    private void sendAudioPacket(short[] audio, boolean whispering) {
        ...
        try {
            if (client != null && client.getRecorder() != null) {
                client.getRecorder().appendChunk(Minecraft.getInstance().getSession().getProfile().getId(), System.currentTimeMillis(), PositionalAudioUtils.convertToStereo(audio));
            }
        } catch (IOException e) {
