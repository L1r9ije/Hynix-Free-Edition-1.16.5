...
public class RotationUtil implements Wrapper {
    ...
    public static Vector3d findPerfectInternalAimPoint(Entity target, double maxDistance) {
        ...
        // Получаем хитбокс цели с небольшим внутренним отступом
        AxisAlignedBB targetBB = getShrunkHitbox(target.getBoundingBox());

        // Вычисляем ближайшую точку внутри уменьшенного хитбокса
        ...
        // Если точка строго внутри (не на границе)
        if (isStrictlyInside(closestPoint, targetBB)) {
            // Проецируем на ближайшую внутреннюю грань
    ...
    private static AxisAlignedBB getShrunkHitbox(AxisAlignedBB original) {
        // Уменьшаем хитбокс со всех сторон на SAFETY_OFFSET
    ...
    private static Vector3d projectToInnerSurface(Vector3d from, AxisAlignedBB box) {
        // Вычисляем расстояния до всех внутренних граней
        double[] distances = {
        ...
        // Проецируем на соответствующую внутреннюю грань
        double x = from.x;
        double y = from.y;
        double z = from.z;
    ...
    private static Vector3d clampToInnerHitbox(Vector3d point, AxisAlignedBB box) {
        // Жестко ограничиваем точку уменьшенным хитбоксом
    ...
    private static Vector3d findClosestSurfacePoint(Vector3d from, AxisAlignedBB box) {
        // Вычисляем расстояния до всех граней
        double[] distances = {
                from.x - box.minX, box.maxX - from.x,
                from.y - box.minY, box.maxY - from.y,
                from.z - box.minZ, box.maxZ - from.z
        ...
        // Создаем точку на ближайшей грани
        double x = from.x;
        double y = from.y;
        double z = from.z;
    ...
    private static Vector3d snapToSurface(Vector3d point, AxisAlignedBB box) {
        // Прижимаем точку к ближайшей поверхности
        double x = MathHelper.clamp(point.x, box.minX, box.maxX);
        double y = MathHelper.clamp(point.y, box.minY, box.maxY);
        double z = MathHelper.clamp(point.z, box.minZ, box.maxZ);
    ...
    private static Vector3d ensureOnSurface(Vector3d point, AxisAlignedBB box) {
        // Гарантируем, что точка точно на поверхности
        ...
        // Если точка не на поверхности, корректируем
        if (!(onX || onY || onZ)) {
    ...
    private static Vector3d rayTraceToBoxSurface(Vector3d start, Vector3d direction, AxisAlignedBB box) {
        // Параметрическое уравнение луча: start + t*direction
    ...
    private static Vector3d correctToBoxSurface(Vector3d point, AxisAlignedBB box) {
        // Корректируем точку, чтобы она точно лежала на поверхности хитбокса
        double x = MathHelper.clamp(point.x, box.minX, box.maxX);
        double y = MathHelper.clamp(point.y, box.minY, box.maxY);
        double z = MathHelper.clamp(point.z, box.minZ, box.maxZ);

        // Определяем, к какой грани ближе точка
        double[] distances = {
        ...
        // Прижимаем точку к ближайшей грани
        switch (closestFace) {
            case 0:
                x = box.minX;
                break;
    ...
    public Vector2f getClosestAimAngles(Entity target) {
        ...
        // и 6 центров граней
        points.add(new Vector3d(midX, midY, minZ)); // передняя грань
        points.add(new Vector3d(midX, midY, maxZ)); // задняя
        points.add(new Vector3d(midX, minY, midZ)); // нижняя
        points.add(new Vector3d(midX, maxY, midZ)); // верхняя
        points.add(new Vector3d(minX, midY, midZ)); // левая
        points.add(new Vector3d(maxX, midY, midZ)); // правая
        ...
        for (Vector3d point : points) {
            // Вычисляем углы наведения на данную точку
            Vector2f aimAngles = calculate(point);
            // Уголовое отклонение между текущим направлением и направлением на point
        ...
        // Если нашли лучшую точку, возвращаем ее углы; иначе — центр цели
        if (bestPoint != null) {
            return calculate(bestPoint);
        } else {
            // На всякий случай: целимся в центр хитбокса
