...
public class FriendManager extends BaseManager<List<FriendManager.FriendEntry>> {
    ...
    protected JsonObject serializeData() {
        ...
        for (FriendEntry friend : data) {
            ...
            friendObj.addProperty("addedTime", friend.addedTime().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME));
    ...
    public void removeFriend(String friendName) {
        ...
        boolean removed = data.removeIf(entry -> entry.name().equalsIgnoreCase(friendName.trim()));
        if (removed) {
            save();
        }
    }
    ...
    public boolean isFriend(String friendName) {
        ...
        return data.stream().anyMatch(entry -> entry.name().equalsIgnoreCase(friendName.trim()));
    }
    ...
    public record FriendEntry(String name, LocalDateTime addedTime) {
