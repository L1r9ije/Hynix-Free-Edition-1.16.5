...
public class FireworkESP extends Module {
    ...
    private void renderFirework(EventRender2D event, Firework firework, Vector2f screenPos, boolean showTime) {
        ...
        if (showTime)
            Fonts.sf_medium[14].drawString(event.getStack(), String.format("%.1f сек.", Math.max(0.0, (firework.getLifetimeMs() - firework.timer.getTimePassed()) / 1000.0)), posX + 11, posY + 4, ColorUtil.getColor(255, 255, 255, (int) (255 * firework.getAlpha())));
    }
