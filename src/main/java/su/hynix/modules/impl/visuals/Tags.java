...
import su.hynix.utils.player.PotionUtil;
...
public class Tags extends Module {
    public MultiBooleanSetting rendering = new MultiBooleanSetting("Отображать", new BooleanSetting("Игроков", true), new BooleanSetting("Монстров", false), new BooleanSetting("Друзей", false), new BooleanSetting("Животных", false), new BooleanSetting("Себя", false), new BooleanSetting("Жителей", false), new BooleanSetting("Голых", false), new BooleanSetting("Предметы", true));
    public BooleanSetting vieweffects = new BooleanSetting("Отображать зелья", false);
    ...
    public BooleanSetting viewitem = new BooleanSetting("Отображать предметы в руках", false);
    public BooleanSetting viewdisplayname = new BooleanSetting("Отображать визуальное имя предмета", false, () -> viewitem.get());
    public BooleanSetting viewvoicemode = new BooleanSetting("Отображать статус VoiceChat", false);

    public Tags() {
        super("Tags", "Отображает имя и информацию об существах", Category.Visuals);
        addSettings(rendering, vieweffects, viewarmor, viewenchants, viewitem, viewdisplayname, viewvoicemode);
    }

    @EventTarget
    public void onRender(EventRender2D event) {
        final float partialTicks = mc.getRenderPartialTicks();
        final StyledFont fontNameCached = Fonts.sf_regular[14];
        final int rectBgColor = ColorUtil.getColor(0, 0, 0, 120);

        for (Entity baseEntity : mc.world.getAllEntities()) {
    ...
    private boolean isEntityTarget(LivingEntity entity) {
        return TargetUtil.isSelfTarget(entity, rendering) || TargetUtil.isPlayerTarget(entity, rendering, true) || TargetUtil.isVillagerTarget(entity, rendering) || TargetUtil.isAnimalTarget(entity, rendering) || TargetUtil.isMonsterTarget(entity, rendering);
    }
    ...
    private void renderItemTag(EventRender2D event, ItemEntity itemEntity, Vector4f b, StyledFont fontName, int rectColor) {
        ...
        IFormattableTextComponent label = new StringTextComponent("").append(nameComp).mergeStyle(rarity.color);
        if (count > 1) {
            label.append(new StringTextComponent(countSuffix));
        }
        String labelPlain = nameComp.getString() + countSuffix;
        float textWidth = fontName.getWidth(labelPlain);
        float fontHeight = fontName.getHeight();
        float centerX = (b.getX() + b.getZ()) / 2f;
        float tagY = b.getY() - fontHeight - 3f;
        RenderUtil.drawMinecraftRectangle(event.getStack(), centerX - textWidth / 2f - 0.5f, tagY - 2f, textWidth + 3.5f, 10f, rectColor);
        fontName.drawText(event.getStack(), label, centerX - textWidth / 2f + 1.5f, tagY + 1.5f, -1);
    }
    ...
    private void renderEntityTag(EventRender2D event, LivingEntity entity, Vector4f b, StyledFont fontName, int rectColor) {
        ...
        boolean isFriend = entity instanceof PlayerEntity && hynix.getInstance().getFriendManager().isFriend(((PlayerEntity) entity).getGameProfile().getName());
        ...
        IFormattableTextComponent labelComponent = new StringTextComponent("").append(nameComponent).append(new StringTextComponent(" ").mergeStyle(TextFormatting.GRAY)).append(new StringTextComponent(Integer.toString(healthValue)).mergeStyle(TextFormatting.RED)).append(new StringTextComponent("HP").mergeStyle(TextFormatting.RED));

        String friendSuffix = isFriend ? " (F)" : "";
        if (isFriend) {
            labelComponent.append(new StringTextComponent(" [").mergeStyle(TextFormatting.GRAY)).append(new StringTextComponent("F").mergeStyle(TextFormatting.GREEN)).append(new StringTextComponent("]").mergeStyle(TextFormatting.GRAY));
        }

        String labelPlain = plainName + " [" + healthValue + "]" + friendSuffix;
        float textWidth = fontName.getWidth(labelPlain);
        float fontHeight = fontName.getHeight();
        float centerX = (b.getX() + b.getZ()) / 2f;
        float tagY = b.getY() - fontHeight - 3f;
        ...
        RenderUtil.drawMinecraftRectangle(event.getStack(), centerX - textWidth / 2f, tagY - 2f, textWidth + 7.5f, 10f, finalRectColor);
        fontName.drawText(event.getStack(), labelComponent, centerX - textWidth / 2f + 2, tagY + 1, -1);

        if (viewvoicemode.get() && entity instanceof PlayerEntity player) {
            drawVoiceChatStatus(event, player, centerX, tagY, textWidth);
        }
        ...
        if (vieweffects.get()) {
            drawEffects(event, entity, centerX, effectsBaseY, Fonts.sf_bold[13]);
        }
    }
                ...
                if (viewenchants.get()) {
                    ...
                    if (!enchantments.isEmpty()) {
                        ...
                        for (String enchantment : enchantments) {
                            ...
                            float enchantWidth = Fonts.sf_medium[12].getWidth(plainText);
                            Fonts.sf_medium[12].drawString(new MatrixStack(), plainText, centerX + offsetX - enchantWidth / 2 + 5, enchantY, -1);
                            enchantY -= 6;
                        }
                    }
                }
    ...
    private float drawHeldItems(EventRender2D event, LivingEntity entity, float centerX, float startY, int rectColor) {
        ...
        if (!off.isEmpty()) {
            ...
            Fonts.sf_regular[12].drawText(event.getStack(), comp, x, y + 1, -1);
            y += lineHeight;
        }
    ...
    private void drawVoiceChatStatus(EventRender2D event, PlayerEntity player, float centerX, float tagY, float textWidth) {
        ...
        if (client != null) {
            ...
            if (isVoiceClient) {
                ...
                int color = manager.isPlayerDisabled(player) ? ColorUtil.hex("#550000") : (isSpeak ? ColorUtil.hex("#00FF00") : ColorUtil.hex("#FF0000"));

                float rectX = centerX - textWidth / 2f;
                float rectY = tagY - 2f;
                float rectWidth = textWidth + 3f;
                float rectHeight = 10f;

                RenderUtil.drawMinecraftRectangle(event.getStack(), rectX - 1f, rectY, 1f, rectHeight, color);
                RenderUtil.drawMinecraftRectangle(event.getStack(), rectX + rectWidth + 0.5f, rectY, 1f, rectHeight, color);
            }
        }
    }
}

record EffectLine(String text, float width, int color) {
}
