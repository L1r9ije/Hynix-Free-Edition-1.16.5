...
public class AutoTrap extends Module {
    ...
    private void onUpdate(EventUpdate e) {
        ...
        if (!ignoreOpenInventory.get() && mc.currentScreen != null && mc.currentScreen != hynix.getInstance().getDropDown())
            return;
    ...
    private boolean isValidTarget(LivingEntity entity) {
        ...
        if (entity instanceof PlayerEntity player) {
            return !hynix.getInstance().getFriendManager().isFriend(player.getGameProfile().getName());
        }
        return true;
    }
