...
public class NoInteract extends Module {
    ...
    private void onUpdate(EventClickBlockRight event) {
        if (noplacesphere.get() && event.getHand() == Hand.OFF_HAND && mc.player.getHeldItemOffhand().getItem() == Items.PLAYER_HEAD)
            event.setCancelled(true);
        if (allBlocks.get()) event.setCancelled(true);
        else if (getBlocks().contains(event.getWorld().getBlockState(event.getResult().getPos()).getBlock()))
            event.setCancelled(true);
    }
}