...
public class ColorPickerElement {
    ...
    public void render(MatrixStack stack, float mouseX, float mouseY) {
        ...
        float alphaOffset = showAlpha ? 0f : -7f;
        float PICKER_WIDTH = 93 + alphaOffset;
        float PICKER_HEIGHT = 90;
        ...
        RenderUtil.drawOutlineRectangle(x + 7, y, PICKER_WIDTH, PICKER_HEIGHT, 4, ThemeEditor.getColor(ThemeSettings.OUTLINE), ThemeEditor.getAlpha(ThemeSettings.OUTLINE));

        RenderUtil.drawRoundedRectangleGradient(x + 11, y + 4.5f, 70, 70, 3, ColorUtil.getColor(255, 255, 255, 255), Color.BLACK.getRGB(), new Color(Color.HSBtoRGB(hue, 1, 1)).getRGB(), Color.BLACK.getRGB(), 1);
        RenderUtil.drawOutlineRectangle(Math.max(x + 13, Math.min(x + 75, x + 11 + saturation * 70)), Math.max(y + 6, Math.min(y + 68, y + 4 + (1 - brightness) * 70)), 4, 4, 1.5f, -1, 255);

        float hueX = x + 84;
        float hueY = y + 5f;
        RenderUtil.drawImage2D(new ResourceLocation("hynix/icons/colorpicker/color.png"), hueX, hueY, 4, 68, -1);
        RenderUtil.drawMinecraftRectangle(stack, hueX - 1, hueY + hue * 68, 4 + 2, 1, -1);

        float alphaX = x + 91 + alphaOffset;
        float alphaY = y + 5;
        if (showAlpha) {
            RenderUtil.drawImage2D(new ResourceLocation("hynix/icons/colorpicker/opacity.png"), alphaX, alphaY, 4, 68, -1);
            RenderUtil.drawMinecraftRectangle(stack, alphaX - 1, alphaY + (1 - alpha) * 68, 4 + 2, 1, -1);
        }
        ...
        RenderUtil.drawRoundedRectangle(x + 11, y + 76, 37, 10, 1.5f, copyBg);
        Fonts.sf_regular[12].drawString(stack, "Копировать", x + 11 + (37 - Fonts.sf_regular[12].getWidth("Копировать")) / 2f, y + 80, copyText);
        ...
        RenderUtil.drawRoundedRectangle(x + 49, y + 76, 37 + alphaOffset, 10, 1.5f, pasteBg);
        Fonts.sf_regular[12].drawString(stack, "Вставить", x + 49 + (alphaOffset / 2) + (37 - Fonts.sf_regular[12].getWidth("Вставить")) / 2f, y + 80, pasteText);
        ...
        RenderUtil.drawRoundedRectangle(x + 87 + alphaOffset, y + 76, 10, 10, 1.5f, pipetteBg);
        Fonts.icons[12].drawString(stack, "R", x + 89 + alphaOffset, y + 81, pipetteText);
        RenderUtil.scaleEnd();
    ...
    public boolean mouseClicked(float mouseX, float mouseY) {
        ...
        if (MathUtil.isHovered(mouseX, mouseY, x + 87 + alphaOffset, y + 76, 10, 10)) {
        ...
        if (MathUtil.isHovered(mouseX, mouseY, x + 11, y + 76, 37, 10)) {
        ...
        if (MathUtil.isHovered(mouseX, mouseY, x + 49 + alphaOffset, y + 76, 37, 10)) {
        ...
        if (MathUtil.isHovered(mouseX, mouseY, x + 11, y + 4, 70, 70)) {
            isDraggingPicker = true;
            updateColorFromMouse(mouseX, mouseY);
            return true;
        }

        if (MathUtil.isHovered(mouseX, mouseY, x + 84 - 1, y + 5, 4 + 2, 68)) {
            isDraggingHue = true;
            updateHueFromMouse(mouseY);
            return true;
        }
        if (showAlpha) {
            if (MathUtil.isHovered(mouseX, mouseY, x + 91 + alphaOffset - 1, y + 5, 4 + 2, 68)) {
    ...
    private void updateColorFromMouse(float mouseX, float mouseY) {
        ...
        float clampedX = Math.max(pickerX, Math.min(mouseX, pickerX + 70));
        float clampedY = Math.max(pickerY, Math.min(mouseY, pickerY + 70));

        saturation = (clampedX - pickerX) / 70;
    ...
    private void updateHueFromMouse(float mouseY) {
        float hueY = y + 5;
        hue = (Math.max(hueY, Math.min(mouseY, hueY + 68)) - hueY) / 68;
    ...
    private void updateAlphaFromMouse(float mouseY) {
        ...
        float alphaY = y + 5;
        alpha = 1 - (Math.max(alphaY, Math.min(mouseY, alphaY + 68)) - alphaY) / 68;
