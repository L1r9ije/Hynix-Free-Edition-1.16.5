...
import su.hynix.events.EventAspectRatio;
import su.hynix.hynix;
import su.hynix.modules.impl.visuals.Removals;
...
public class ProjectUtil implements Wrapper {
    ...
    public static Vector2f project2D(double x, double y, double z) {
        ...
        if (renderViewEntity instanceof PlayerEntity playerentity) {
            if (!hynix.getInstance().getModuleManager().removals.isEnabled() || !Removals.mode.is("Тряска камеры"))
