...
public class BindCommand extends Command implements Wrapper {
    ...
    private int executeAdd(CommandContext<ISuggestionProvider> context) {
        ...
        ModuleManager mm = hynix.getInstance().getModuleManager();
    ...
    private int executeList(CommandContext<ISuggestionProvider> context) {
        List<Module> bound = hynix.getInstance().getModuleManager().getModules().stream().filter(m -> m.getBind() != -100).collect(Collectors.toList());
        ...
        for (Module m : bound) {
            ...
            String removeCmd = hynix.getInstance().getCommandsManager().getPrefix() + getCommand() + " remove " + normalizeModuleName(m.getName());
    ...
    private int executeClear(CommandContext<ISuggestionProvider> context) {
        boolean anyBound = hynix.getInstance().getModuleManager().getModules().stream().anyMatch(m -> m.getBind() != -100);
        if (!anyBound) {
        ...
        hynix.getInstance().getModuleManager().getModules().forEach(m -> m.setBind(-100));
    ...
    private CompletableFuture<Suggestions> suggestModuleNames(CommandContext<ISuggestionProvider> ctx, SuggestionsBuilder builder) {
        List<String> names = hynix.getInstance().getModuleManager().getModules().stream().map(Module::getName).map(this::normalizeModuleName).collect(Collectors.toList());
    ...
    private CompletableFuture<Suggestions> suggestAdd(CommandContext<ISuggestionProvider> ctx, SuggestionsBuilder builder) {
        ...
        if (parts.length == 0 || (parts.length == 1 && !hasTrailingSpace)) {
            ...
            List<String> names = hynix.getInstance().getModuleManager().getModules().stream().map(Module::getName).map(this::normalizeModuleName).filter(n -> n.toLowerCase(Locale.ROOT).startsWith(prefix.toLowerCase(Locale.ROOT))).collect(Collectors.toList());
