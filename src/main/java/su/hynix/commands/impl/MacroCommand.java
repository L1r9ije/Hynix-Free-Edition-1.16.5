...
public class MacroCommand extends Command implements ArgumentType<String>, Wrapper {
    ...
    public void run(LiteralArgumentBuilder<ISuggestionProvider> builder) {
        ...
        builder.then(literal("remove").then(args("name", StringArgumentType.word()).suggests(this::suggestMacroNames).executes(ctx -> {
            ...
            hynix.getInstance().getMacroManager().removeMacro(name);
        ...
        builder.then(literal("list").executes(ctx -> {
            List<MacroManager.MacroEntry> macros = hynix.getInstance().getMacroManager().getMacros();
            if (macros.isEmpty()) {
            ...
            for (MacroManager.MacroEntry m : macros) {
                String key = KeyMapper.getKey(m.keyCode());
                IFormattableTextComponent nameLbl = new StringTextComponent("Имя: ").setStyle(Style.EMPTY.applyFormatting(TextFormatting.GRAY));
                IFormattableTextComponent nameVal = new StringTextComponent(m.name()).setStyle(Style.EMPTY.applyFormatting(TextFormatting.WHITE));
                ...
                IFormattableTextComponent textVal = new StringTextComponent(m.command()).setStyle(Style.EMPTY.applyFormatting(TextFormatting.GREEN));
                String removeCmd = hynix.getInstance().getCommandsManager().getPrefix() + getCommand() + " remove " + m.name();
        ...
        builder.then(literal("clear").executes(ctx -> {
            ...
            hynix.getInstance().getMacroManager().clear();
    ...
    private int executeAdd(CommandContext<ISuggestionProvider> ctx) {
        ...
        hynix.getInstance().getMacroManager().addMacro(name, key.getKeyCode(), command);
    ...
    private CompletableFuture<Suggestions> suggestMacroNames(CommandContext<ISuggestionProvider> ctx, SuggestionsBuilder builder) {
        List<String> names = hynix.getInstance().getMacroManager().getMacros().stream().map(MacroManager.MacroEntry::name).collect(Collectors.toList());
