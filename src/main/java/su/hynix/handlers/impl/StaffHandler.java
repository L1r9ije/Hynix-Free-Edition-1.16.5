...
import su.hynix.modules.impl.player.AutoLeave;
...
public class StaffHandler implements Wrapper {
    ...
    public void onUpdate(EventUpdate e) {
        ...
        if ((hynix.getInstance().getModuleManager().getModule(Interface.class).isEnabled() && Interface.elements.is("Администрация онлайн")) || hynix.getInstance().getModuleManager().getModule(AutoLeave.class).isEnabled())
            update();
    }
    ...
    public void update() {
        ...
        online.sort(Comparator.comparing(Staff::name, String.CASE_INSENSITIVE_ORDER));
        vanished.sort(Comparator.comparing(Staff::name, String.CASE_INSENSITIVE_ORDER));
        synchronized (staff) {
            staff.clear();
            staff.addAll(online);
    ...
    private List<Staff> getOnlineStaff() {
        if (mc.world == null || mc.getConnection() == null) return Collections.emptyList();
        StaffManager staffManager = hynix.getInstance().getStaffManager();
        Set<String> staffNamesLower = staffManager != null ? staffManager.getStaff().stream().map(StaffManager.StaffEntry::name).filter(Objects::nonNull).map(name -> name.toLowerCase(LOCALE)).collect(Collectors.toSet()) : Collections.emptySet();
