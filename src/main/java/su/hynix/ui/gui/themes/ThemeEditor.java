...
import su.hynix.managers.impl.ClientConfig;
import su.hynix.managers.impl.ThemeManager;
import su.hynix.modules.api.constructors.impl.ColorSetting;
import su.hynix.modules.api.constructors.impl.StringSetting;
import su.hynix.modules.api.elements.impl.ColorElement;
...
public class ThemeEditor extends Panel {
    ...
    private final AnimationUtil createAnimation = new AnimationUtil(0f, 6f, Easings.LINEAR);
    ...
    private void updateAndSavePreset() {
        if (currentPreset != null && currentPreset.isCustom()) {
            ...
            String creator = "HynixUser";
            String cleanPresetName = currentPreset.name().replace("Имя: ", "");
            currentPreset = new ThemePreset(currentPreset.name(), updatedColors, creator, true);
            customPresets.put(currentPreset.name(), updatedColors);
    ...
    private void initDefaultPresets() {
        ...
        String presetToUse = "Имя: Прозрачная";
    ...
    private int renderPresetList(MatrixStack stack, float mouseX, float mouseY, Map<String, int[]> presets, int startIndex, float startX, float startY) {
        for (Map.Entry<String, int[]> preset : presets.entrySet()) {
            if (startIndex >= 11) break;

            float circleX = startX + (presetCircleRadius * 2 + presetCircleSpacing) * (startIndex % 11) + 4;
            float circleY = getY() + 40 + ((startIndex >= 11) ? (presetCircleRadius * 2 + presetCircleSpacing) : 0) - 8;
...
//            RenderUtil.drawRoundedRectangle(circleX - presetCircleRadius, circleY - presetCircleRadius, presetCircleRadius * 2, presetCircleRadius * 2, presetCircleRadius - 2, preset.getValue()[ThemeSettings.LOGO.ordinal()]);

            int color = preset.getValue()[ThemeSettings.LOGO.ordinal()];

            String activeTheme = ClientConfig.getCurrentActiveTheme();
            if (preset.getKey().equals(activeTheme)) {
                color = ColorUtil.getColor(255, 255, 255);
//                RenderUtil.drawRoundedRectangle(circleX - selectedCircleRadius, circleY - selectedCircleRadius, selectedCircleRadius * 2, selectedCircleRadius * 2, selectedCircleRadius - 1, ColorUtil.getColor(0, 0, 0, 255));
            }

            RenderUtil.drawRoundedRectangle(circleX - presetCircleRadius + selectedCircleRadius + 0.5f, circleY - presetCircleRadius + 0.5f, selectedCircleRadius * 2, selectedCircleRadius * 2, selectedCircleRadius - 1, preset.getValue()[ThemeSettings.LOGO.ordinal()]);
            RenderUtil.drawOutlineRectangleBold(
                    circleX - presetCircleRadius + selectedCircleRadius - 1f,
                    circleY - presetCircleRadius - 1,
                    (selectedCircleRadius * 2) + 3,
                    (selectedCircleRadius * 2) + 3,
                    3.25f,
                    color, ColorUtil.alpha(preset.getValue()[ThemeSettings.LOGO.ordinal()])
            );
            ...
            float tooltipAlpha = anim.getValue();
            if (tooltipAlpha > 0) {
                String name = preset.getKey();
                String author = presets == defaultPresets ? "hynix" : "HynixUser";
                float nameWidth = Fonts.sf_regular[12].getWidth(name);
                float authorWidth = Fonts.sf_regular[12].getWidth("Автор: " + author);
                float textWidth = Math.max(nameWidth, authorWidth);
                float tooltipX = circleX - presetCircleRadius + 2;
                float tooltipY = circleY - presetCircleRadius - 12f - 4f;

                RenderUtil.drawBlurredRoundedRectangle(tooltipX - 2, tooltipY - 4, textWidth + 4, 16, 2, getColor(ThemeSettings.TOOLTIP_BG), tooltipAlpha);
                int textColorWithAlpha = ColorUtil.applyOpacity(getColor(ThemeSettings.TEXT), (int) (tooltipAlpha * 255));
                Fonts.sf_regular[12].drawString(stack, name, tooltipX, tooltipY - 0.5f, textColorWithAlpha);
                Fonts.sf_regular[12].drawString(stack, "Автор: " + author, tooltipX, tooltipY + 6.5f, textColorWithAlpha);
            }
    ...
    public void render(MatrixStack stack, float mouseX, float mouseY, float alpha) {
        ...
        float contentHeight = getHeight() - 17;
        ...
        RenderUtil.drawBlurredRoundedRectangle(x, y + 4, getWidth(), getHeight() - 4, 8, ThemeEditor.getColor(ThemeSettings.MAIN), 1);
        RenderUtil.drawOutlineRectangle(x, y + 4, getWidth(), getHeight() - 4, 8, getColor(ThemeSettings.OUTLINE), getAlpha(ThemeSettings.OUTLINE));


        float textWidth = Fonts.sf_medium[26].getWidth("Themes");
        float textHeight = Fonts.sf_medium[26].getHeight();
        int startX = (int) (x + (getWidth() - textWidth) / 2);
        int startY = (int) (y - textHeight / 2 + 13);
        ITextComponent gradientText = GradientUtil.gradient("f", ColorUtil.gradientInterface(90), ThemeEditor.getColor(ThemeSettings.LOGO), 70, 45);

        Fonts.sf_medium[19].drawString(stack, "Themes", getX() + 8, getY() + 12 + 1, ThemeEditor.getColor(ThemeSettings.HEADER));
        Fonts.hynix[19].drawText(stack, gradientText, getX() + getWidth() - 15, getY() + 12 + 1, -1);

        for (StringElement stringElement : stringElements) {
            stringElement.setX(x + 1);
            stringElement.setY(y + 36);
            stringElement.setWidth(getWidth() - 24);
            stringElement.render(stack, mouseX, mouseY, alpha);
        }

        float buttonX = x + 80;
        float buttonY = y + 36 + 4;
        ...
        RenderUtil.drawRoundedRectangle(buttonX, buttonY, buttonWidth, buttonHeight, 2, createBg);
        RenderUtil.drawOutlineRectangle(buttonX, buttonY, buttonWidth, buttonHeight, 2.5f, getColor(ThemeSettings.OUTLINE), getAlpha(ThemeSettings.OUTLINE));
        float textX = buttonX + (buttonWidth - Fonts.sf_regular[12].getWidth("Создать")) / 2f;
        float textY = buttonY + 5.5f;
        int createTextColor = ColorUtil.interpolate(getColor(ThemeSettings.TEXT), getColor(ThemeSettings.TEXT_INACTIVE), createAnim);
        Fonts.sf_regular[12].drawString(stack, "Создать", textX, textY, createTextColor);

        float pickerY = y + 60;

        ScissorUtil.start(x, y + 55, getWidth(), getHeight() - 57);
...
//            RenderUtil.drawRoundedRectangle(colorPicker.getX() + 2.5f, colorPicker.getY() - 3.5f, colorPicker.getWidth() - 5.5f, colorPicker.getHeight(), 2, ThemeEditor.getColor(ThemeSettings.FIELD_INACTIVE));
            colorPicker.render(stack, mouseX, mouseY, alpha);

            pickerY += colorPicker.getHeight();
        }
        ...
        if (maxHeight > contentHeight) {
            float thumbHeight = Math.max(20f, (contentHeight / maxHeight) * getHeight() - 66);
            float thumbPosition = (-animatedScroll / (maxHeight - contentHeight)) * (getHeight() - 66 - thumbHeight);
        }
    ...
    public void mouseClicked(float mouseX, float mouseY, int button) {
        ...
        if (contextMenu != null) {
            if (button == 0) {
                ...
                float halfH = menuH / 2f;
                if (MathUtil.isHovered(mouseX, mouseY, menuX, menuY, menuW, halfH)) {
                    ...
                    if (presetName != null && customPresets.containsKey(presetName)) {
                        ...
                        if (currentPreset != null && currentPreset.name().equals(presetName)) {
                            List<Map.Entry<String, int[]>> allPresets = new ArrayList<>(defaultPresets.entrySet());
                            allPresets.addAll(customPresets.entrySet());
                            int removedIndex = contextMenu.getPresetIndex();
                            int fallbackIndex = Math.max(0, removedIndex - 1);
                            Map.Entry<String, int[]> fallback = allPresets.get(fallbackIndex);
                            applyPreset(fallback.getValue(), fallback.getKey(), customPresets.containsKey(fallback.getKey()));
                        }
                    }
                } else if (MathUtil.isHovered(mouseX, mouseY, menuX, menuY + halfH, menuW, halfH)) {
                    String presetName = getPresetNameByIndex(contextMenu.getPresetIndex());
                    if (presetName != null && customPresets.containsKey(presetName)) {
                        String cleanName = presetName.replace("Имя: ", "");
                        File themeFile = new File(FilePath.BASE_PATH + FilePath.THEMES_DIR_REL, cleanName + ".file");
                        try {
        ...
        if (MathUtil.isHovered(mouseX, mouseY, x + 84, y + 40, 30, 12) && button == 0) {
        ...
        for (Map.Entry<String, int[]> preset : allPresets) {
            ...
            float circleY = getY() + 40.5f + ((index >= 11) ? (presetCircleRadius * 2 + presetCircleSpacing) : 0) - 9;
            ...
            if (MathUtil.isHovered(mouseX, mouseY, circleX - presetCircleRadius, circleY - presetCircleRadius, presetCircleRadius * 2, presetCircleRadius * 2)) {
                ...
                if (button == 0) {
                    ...
                    contextMenu = new ThemePresetContextMenu(circleX + 2, circleY - presetCircleRadius - 1, 52, 28, index);
                }
                return;
            }
    ...
    private void applyPreset(int[] colors, String presetName, boolean isCustom) {
        ...
        currentPreset = new ThemePreset(presetName, colors, "HynixUser", isCustom);
    ...
    private static class ThemePresetContextMenu {
        ...
        public void render(MatrixStack stack) {
            ...
            RenderUtil.drawBlurredRoundedRectangle(x, y + 4, width, height, 4, getColor(ThemeSettings.MAIN), 1);
            RenderUtil.drawOutlineRectangle(x, y + 4, width, height, 4, getColor(ThemeSettings.OUTLINE), getAlpha(ThemeSettings.OUTLINE));

            float textX = x + 13;

            RenderUtil.drawImage2D(new ResourceLocation("hynix/icons/gui/bin.png"), x + 3f, y + 7.5f, 8, 8, getColor(ThemeSettings.TEXT));
            Fonts.sf_semibold[12].drawString(stack, "Удалить", textX - 0.5f, y + 10.5f, getColor(ThemeSettings.TEXT));

            RenderUtil.drawRoundedRectangle(x + 2, y + 17.5f, 48, 1, 1.5f, getColor(ThemeSettings.SEPARATOR));

            RenderUtil.drawImage2D(new ResourceLocation("hynix/icons/gui/copy.png"), x + 3.5f, y + 20.5f, 8, 8, getColor(ThemeSettings.TEXT));
            Fonts.sf_semibold[12].drawString(stack, "Копировать", textX, y + 23.5f, getColor(ThemeSettings.TEXT));
            RenderUtil.scaleEnd();
        }
    ...
    public record ThemePreset(String name, int[] colors, String creator, boolean isCustom) {
