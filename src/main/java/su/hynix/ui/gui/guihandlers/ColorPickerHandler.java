package su.hynix.ui.gui.guihandlers;
...
import su.hynix.modules.api.elements.impl.ColorPickerElement;
        ...
        for (Panel panel : panels) {
            ...
            if (panel instanceof ThemeEditor themeEditor) {
                for (ColorElement ce : themeEditor.getColorPickers()) {
                    ...
                    float scissorTop = themeEditor.getY() + 60;
                    float scissorBottom = themeEditor.getY() + themeEditor.getHeight() + 13;
                    if (ce.isVisible() && button == 0 && (themeEditor.getCurrentPreset() == null || themeEditor.getCurrentPreset().isCustom()) && MathUtil.isHovered(mouseX, mouseY, ce.getX() + 2.5f, ce.getY() - 3.5f, ce.getWidth() - 5.5f, ce.getHeight()) && effectiveY >= scissorTop && effectiveY + ce.getHeight() <= scissorBottom) {
